...
export default function App() {
  ...
  const handleStationClick = useCallback(
    (station: Station) => {
      ...
      void audioEngine.play(url, station.stationuuid, station).catch(() => {
        // Auto-retry once on Android if first play fails (AudioContext cold start)
        setTimeout(() => {
          if (useStore.getState().player.currentStation?.stationuuid === station.stationuuid && !audioEngine.isPlaying()) {
            void audioEngine.play(url, station.stationuuid, station).catch(() => {});
          }
        }, 300);
      });
  ...
  // Keep audio alive when app goes to background (Android)
  useEffect(() => {
    const onVisibility = () => {
