...
const GlobeView = memo(function GlobeView({ stations, onStationClick }: GlobeViewProps) {
  ...
  // Rebuild points layers when data or active station changes (only after globe is ready)
  useEffect(() => {
    if (!globeReadyRef.current) return;
    const timer = setTimeout(rebuildPointsLayers, 50);
  ...
  const handleGlobeReady = useCallback(() => {
    ...
    // Mark globe as ready and build circle points layers
